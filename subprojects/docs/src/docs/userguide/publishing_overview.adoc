...
+
In addition, <<publishing_maven.adoc#publishing_maven:resolved_dependencies,Maven>> and <<publishing_ivy.adoc#publishing_ivy:resolved_dependencies,Ivy>> publications support publishing _resolved_ versions of dependencies instead of _declared_ ones.
...
== Adding optional artifacts to a publication

If you have some artifacts not always necessary, you can configure them as `optional`. The optional artifacts will be ignored if their files don't exist when publishing. This is useful when the task that produces the artifact can be set as disabled or the file to published only exists in some situations.

.Attaching an optional PublishArtifact to a publication
====
include::sample[dir="maven-publish/optional-artifact/groovy",files="build.gradle[tags=publish-optional-artifact]"]
include::sample[dir="maven-publish/optional-artifact/kotlin",files="build.gradle.kts[tags=publish-optional-artifact]"]
====
