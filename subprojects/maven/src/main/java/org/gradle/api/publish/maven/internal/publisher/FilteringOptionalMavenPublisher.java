...
/**
 * A MavenPublisher that filter all the invalid artifacts whose optional is true.
 */
public class FilteringOptionalMavenPublisher implements MavenPublisher {
    private final MavenPublisher delegate;

    public FilteringOptionalMavenPublisher(MavenPublisher delegate) {
    ...
    private static class FilteringMavenNormalizedPublication implements MavenNormalizedPublication {
        ...
        public MavenArtifact getPomArtifact() {
            ...
            if (pomArtifact.getOptional() && !pomArtifact.getFile().exists()) {
                throw new IllegalArgumentException("pom artifact cannot be optional");
            }
        ...
        public MavenArtifact getMainArtifact() {
            ...
            if (mainArtifact != null && mainArtifact.getOptional() && !mainArtifact.getFile().exists()) {
                return null;
            }
        ...
        public Set<MavenArtifact> getAdditionalArtifacts() {
            return delegate.getAdditionalArtifacts().stream().filter(mavenArtifact -> {
                return !mavenArtifact.getOptional() || mavenArtifact.getFile().exists();
            }).collect(Collectors.toSet());
        }

        @Override
        public Set<MavenArtifact> getAllArtifacts() {
            return delegate.getAllArtifacts().stream().filter(mavenArtifact -> {
                return !mavenArtifact.getOptional() || mavenArtifact.getFile().exists();
            }).collect(Collectors.toSet());
        }
    }
